...
DATA_PATH=../UE_memory_transformer/data/quality_binary
...
MODEL_PATH=./runs/finetune/quality_bc/$MODEL_NAME/${rmt_params}_lr${LR}_${SCHEDULER}_adamw_wd${WD}_bs${TBS}_p${PATIENCE}_len${SRC_LEN}_it${ITERS}/run_$N
echo $MODEL_PATH
horovodrun --gloo -np $NP python -m downstream_tasks.quality_bc.run_finetuning_quality_bc_rmt \
        --data_path ${DATA_PATH}/train.jsonl \
        --valid_data_path ${DATA_PATH}/validation.jsonl \
