...
echo RUNNING: TASK_NAME SRC_LEN MODEL_NAME MEMORY_SIZE INPUT_SEQ_LEN SEG_SIZE LR N
echo RUNNING: $TASK_NAME $SRC_LEN $MODEL_NAME $MEMORY_SIZE $INPUT_SEQ_LEN 256 $LR $N
horovodrun --gloo -np $NP python run_finetuning_scrolls_rmt.py \
        --task_name $TASK_NAME \
        --model_path ../runs/finetune/debug/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${TGT_LEN}_mem${MEMORY_SIZE}_bs${TBS}_iters${ITERS}_sl_rep_first_seg256/run_$N \
