...
MODEL_CLS=modeling_rmt_enc_dec_debug2:RMTEncoderDecoderForConditionalGeneration
...
INPUT_SEQ_LENS=(1002 1503 25)
MEMORY_SIZES=(10 10 25)
...
echo RUNNING: $TASK_NAME $SRC_LEN $MODEL_NAME $MEMORY_SIZE $INPUT_SEQ_LEN $LR $N
horovodrun --gloo -np $NP python run_finetuning_scrolls_rmt_debug2.py \
        --task_name $TASK_NAME \
        --model_path ../runs/finetune/debug/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${TGT_LEN}_mem${MEMORY_SIZE}_bs${TBS}_iters${ITERS}_sl_rep_first_pad_right/run_$N \
