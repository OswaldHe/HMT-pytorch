...
for SEGMENT_ORDERING in regular repeat_first bidirectional
do
...
horovodrun --gloo -np $NP python run_finetuning_scrolls_rmt.py \
        --task_name $TASK_NAME \
        --model_path ../runs/finetune/debug/$TASK_NAME/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SIZE}_mem${MEMORY_SIZE}_sl_ilm_${SEGMENT_ORDERING}/run_$N \
