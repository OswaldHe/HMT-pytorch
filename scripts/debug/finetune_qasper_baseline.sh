...
horovodrun --gloo -np $NP python run_finetuning_scrolls.py \
        --task_name $TASK_NAME \
        --model_path ../runs/finetune/debug/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${SRC_LEN}-${TGT_LEN}_bl_bs${TBS}_iters${ITERS}_sl/run_$N \
        ...
        --lr ${LR} --lr_scheduler $SCHEDULER --num_warmup_steps 160 \
        --data_n_workers 2 \
