...
MAX_N_SEGMENTSS=(3 4 5 6 7 8)
MEMORY_SIZES=(1 1 1 1 1 1)
BSS=(4 2 2 2 1 1)
LRS=(5e-05 5e-05 5e-05 1e-05 1e-05 1e-05)
...
horovodrun --gloo -np $NP python run_finetuning_lm_multiseg_rmt_chunked.py \
        ...
        --model_cls $MODEL_CLS \
        --model_cpt ../runs/lm_long/${TASK_NAME}/$MODEL_NAME/${SCHEDULER}_adamw_wd1e-03_$(((INPUT_SIZE-2*MEMORY_SIZE)*(MAX_N_SEGMENTS-1)))-${TGT_LEN}-$((MAX_N_SEGMENTS-1))x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_from_cpt_$((MAX_N_SEGMENTS-2))-$((MAX_N_SEGMENTS-1))_segm_valid/run_1 \
