...
class Trainer:
    ...
    def step(self, batch, is_train_mode=True, save_attentions=False) -> Tuple[Dict[str, float], Dict[str, list]]:
        ...
        with torch.set_grad_enabled(is_train_mode):
            for j in range(0, batch_size, self.args.batch_size):
                ...
                if save_attentions:
                    ...
                    _layer = -1
                    size = 20
                    outputs = self.model(**{k: subbatch[k] for k in subbatch if k in self.model_forward_args}, output_attentions=True)
                    attention_map = outputs['encoder_attentions'][_layer][_b, _head].cpu().detach()
                    corner_map = attention_map[:size, :size]
                    
                    img_folder = '/home/bulatov/bulatov/RMT_light/t5-experiments/images/'
                    img_folder += f'mem_{self.model.num_mem_tokens}/'
                    if not os.path.exists(img_folder):
                        os.system(f'mkdir {img_folder}')
                        os.system(f'mkdir {img_folder}/full')
                        os.system(f'mkdir {img_folder}/corner')

                    # full map
                    fig = plt.figure(figsize=(12,12))
                    ax = fig.add_subplot(1, 1, 1)
                    ax.matshow(attention_map, cmap='Reds')
                    plt.savefig(img_folder + f'full/{self.n_iter}.png')

                    # corner
                    x_tokens = ['m'] * self.model.num_mem_tokens  +  [''] * (size - self.model.num_mem_tokens)
                    fig = plt.figure(figsize=(12,12))
                    ax = fig.add_subplot(1, 1, 1)
                    ax.matshow(corner_map, cmap='Reds')

                    ax.set_xticks(range(size))
                    ax.set_yticks(range(size))
                    ax.set_xticklabels(x_tokens)
                    ax.set_yticklabels(x_tokens)

                    plt.savefig(img_folder + f'corner/{self.n_iter}.png')
    ...
    def train(self) -> None:
        ...
        for batch in train_batches:
            ...
            # save_attentions = self.args.log_interval and self.n_iter % self.args.log_interval == 0
    ...
    def validate(self, dataloader, split='valid', write_tb=True) -> Dict[str, float]:
        ...
        for batch in dataloader:
            batch_metrics, batch_metrics_data = self.step(batch, is_train_mode=False, save_attentions=save)
            save = False
