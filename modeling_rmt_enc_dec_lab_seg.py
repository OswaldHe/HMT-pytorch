...
class RMTEncoderDecoderForConditionalGeneration():
    ...
    def set_params(self, 
        ...
        self.num_special_tokens_to_add = tokenizer.num_special_tokens_to_add() + 1 # !!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def extend_word_embeddings(self):
        ...
        extended_vocab_size = vocab_size + self.num_mem_tokens + max_n_segments
        self.mem_token_ids = torch.arange(vocab_size, vocab_size + self.num_mem_tokens)
        # self.seg_token_ids = [torch.tensor([i]) for i in range(vocab_size + self.num_mem_tokens, vocab_size + self.num_mem_tokens + max_n_segments)]
        self.seg_token_ids = torch.arange(vocab_size + self.num_mem_tokens, vocab_size + self.num_mem_tokens + max_n_segments)
        # print('\n\n\n\nself.seg_token_ids', self.seg_token_ids)
        self.resize_token_embeddings(extended_vocab_size)
        self.embeddings = self.encoder.embed_tokens
        # self.seg_tokens = self.embeddings(self.seg_token_ids)
    ...
    def pad_and_segment(self, input_ids):
        ...
        for input in input_ids:
            ...
            def pad_add_special_tokens(tensor, seg_size, seg_num):
                input_elements = []
                if self.bos_token is not None:
                    input_elements.append(self.bos_token.to(device=self.device))

                if seg_num is not None:
                    seg_token = self.seg_token_ids[seg_num:seg_num+1].to(device=self.device)
                    input_elements += [seg_token]
                ...
                # print('input_elements ', [len(i) for i in input_elements])
                tensor = torch.cat(input_elements)
                if tensor.shape[0] > self.input_size:
                    print(f'\n\n\n!!!!segment is larger than input: {tensor.shape[0]}')
                    print('input_elements ', [len(i) for i in input_elements])
            ...
            if len(input_segments) > 2:
                print('\n\n\n!!!!!!!!!!!!!!!!!!!!!!\nlen(input_segments) ', len(input_segments))
            input_segments = [pad_add_special_tokens(t, self.input_size, sn) for sn, t in enumerate(input_segments)]
            empty = torch.Tensor([]).int()
            self.empty = pad_add_special_tokens(empty, self.input_size, None)
