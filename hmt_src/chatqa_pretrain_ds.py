...
def ChatQAPretrain(dataset, tokenizer, batch_size=1, shuffle=False, clip=False, max_len=3000, seed=3704):
    ...
    def tokenize_function(examples):
        ...
        for ind in range(len(examples['document'])):
            e = {"document": examples['document'][ind], "messages": examples['messages'][ind], "answers": examples['answers'][ind]}
            context = e['document']
            question = e['messages']
            answer = e["answers"]
    ...
    tokenized_dataset = dataset.map(
        ...
        desc='tokenize ChatQA Pretrain dataset',
        num_proc=32
    )
