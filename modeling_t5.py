...
""" PyTorch T5 model. """
""" Copy-pasted from Transformers 4.0.1 """
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        attn_heads = unshape(torch.matmul(attn_weights, value_states))  # (batch_size, seq_length, dim)
        attn_output = self.o(attn_heads)
        ...
        if output_attentions:
            ...
            if output_head_hiddens:
                outputs = outputs + ((attn_heads, attn_weights),)
            else:
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        if config.is_decoder and hasattr(config, 'decoder_self_attention_layer'):
            Attention = get_cls_by_name(config.decoder_self_attention_layer)
        elif not config.is_decoder and hasattr(config, 'encoder_self_attention_layer'):
            Attention = get_cls_by_name(config.encoder_self_attention_layer)
        else:
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.is_decoder and hasattr(config, 'decoder_cross_attention_layer'):
            Attention = get_cls_by_name(config.decoder_cross_attention_layer)
        else:
            Attention = T5Attention
...
class T5CDQAttention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.is_decoder = config.is_decoder
        if self.is_decoder:
            raise RuntimeError('T5CDQAttention is not implemented for decoder cause casual masking!')
        self.has_relative_attention_bias = has_relative_attention_bias
        ...
        # n_heads vectors T = [t1; t2, ...] that are used to compute attention over H to produce P = [p1; p2; ...]
        self.T = nn.Linear(self.d_model, self.cdq_n_heads, bias=False)
        self.W_t = nn.Linear(self.d_model, self.d_model, bias=False)

        # k, v are shared between MHA and CDQ-MHA -> 1 head from MHA and 1 head from CDQ-MHA will have the same K and V
        # todo: maybe not to share?
        # todo: make k,v also context-dependent as q?
        self.k = nn.Linear(self.d_model, self.inner_dim, bias=False)
        self.v = nn.Linear(self.d_model, self.inner_dim, bias=False)
    ...
    def forward(
        ...
        # T = [t1,t2,t3,..,t_{n_heads}], T x relu(W_t x H) = T_att_scores # (bs, n_heads, seq_len)
        # T x relu(W_t x H) aka multiplicative attention with non-linearity
        # todo: try without relu, or with relu(T)
        T_att_scores = self.T(F.relu(self.W_t(hidden_states))).transpose(1, 2)
        if mask is not None:
            if self.is_decoder:
                # wow, we are in decoder now
                print('Hey, Decoder!')
                # if it is not SA ignore mask
                # SA mask has shape (bs, 1, dec_len, dec_len)
                # CA mask has shape (bs, 1, 1, enc_len)
                if mask.shape[-1] == T_att_scores.shape[-1]:
                    ...
                    print('wow, how to use u, mask?')
            else:
                # mask: (bs, 1, 1, seq_len)
                T_att_scores += mask.view(batch_size, 1, seq_length)

        T_attn_weights = F.softmax(T_att_scores.float(), dim=-1).type_as(T_att_scores)
        T_attn_weights = F.dropout(T_attn_weights, p=self.dropout, training=self.training)
        P = torch.matmul(T_attn_weights, hidden_states)  # (bs, n_heads, model_dim)
        # PH = (bs, n_heads, seq_len, d_model x 2)
        PH = torch.cat([
