...
class RMTEncoderForSequenceClassification():
    ...
    def set_params(self, model_attr='bert', drop_empty_segments=True,
    input_size=None, input_seg_size=None, num_mem_tokens=0, bptt_depth=-1, pad_token_id=0, cls_token_id=101, sep_token_id=102):
    ...
    def __call__(self, input_ids, **kwargs):
        ...
        for seg_num, segment_data in enumerate(zip(*segmented)):
            ...
            if self.drop_empty_segments:
                ...
                non_empty_ind = [not torch.equal(input_ids[i], self.empty) for i in range(len(input_ids))]
                # print(f'{sum(non_empty_ind)} non-empty segments at step {seg_num}: {non_empty_ind}')
                input_ids = input_ids[non_empty_ind]
                attention_mask = attention_mask[non_empty_ind]
                token_type_ids = token_type_ids[non_empty_ind]
                seg_kwargs['labels'] = seg_kwargs['labels'][non_empty_ind]

                inputs_embeds = self.net.embeddings.word_embeddings(input_ids)
                inputs_embeds[:, 1:1+self.num_mem_tokens] = memory[non_empty_ind]
            ...
            if self.drop_empty_segments:
                memory[non_empty_ind] = out.hidden_states[-1][:, :self.num_mem_tokens]
            else:
