...
class RMTEncoderDecoderForConditionalGeneration():
    ...
    def __call__(self, input_ids, **kwargs):
        ...
        ## calc memory gradients
        gradients = {}
        for num_segments in range(1, 1 + len(segmented)):
            # print(f'forwarding first {num_segments} segments')
            for n, p in self.named_parameters():
                # print(f'setting to None grad {n}')
                if p.requires_grad:
                    p.grad = None
            ...
            for seg_num, segment_data in enumerate(segmented[:num_segments]):
            ...
            if self.training:
                ...
                # print('writing grad')
                loss.backward()
            # print('\n\nself.embeddings.weight.grad shape', self.embeddings.weight.grad.shape)
                mem_grad = self.embeddings.weight.grad[self.mem_token_ids]
                # print(f'\n\nmem grad seg {num_segments}', mem_grad)
                gradients[f'grad_mean_seg{num_segments}'] = mem_grad.mean()
                gradients[f'grad_abs_seg{num_segments}'] = mem_grad.abs().mean()
