    ...
    def generate(self, input_ids, **kwargs):
        ...
        for seg_num, segment_data in enumerate(segmented):
            ...
            if seg_num < len(segmented)-1:
                ...
                fwd_out = self.model.forward(**seg_kwargs, output_hidden_states=True, labels=labels)
                
                if self.drop_empty_segments:
                    memory[non_empty_mask] = fwd_out.encoder_hidden_states[-1][:, :self.num_mem_tokens]
                else:
                    memory = fwd_out.encoder_hidden_states[-1][:, :self.num_mem_tokens]
            else:
